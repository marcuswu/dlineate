...
func (g *GraphCluster) AddElement(e el.SketchElement) {
	...
	utils.Logger.Debug().Msgf("Cluster %d adding element %d", g.id, e.GetID())
...
func (g *GraphCluster) localSolve() solver.SolveState {
	...
	for _, c := range g.constraints {
		...
		utils.Logger.Info().Msgf("Solving constraint %d betw first two elements: %d, %d", c.GetID(), e1, e2)
		state = solver.SolveConstraint(c)
		utils.Logger.Trace().Msgf("State: %v", state)
	...
	*/
	...
	for len(g.solveOrder) > 0 {
		...
		utils.Logger.Trace().Msgf("Solve Order: %v", g.solveOrder)
		e := g.solveOrder[0]
		g.solveOrder = g.solveOrder[1:]
		c := g.unsolvedConstraintsFor(e)

		if len(g.solvedConstraintsFor(e)) >= 2 {
			utils.Logger.Trace().Msgf("Element %d already solved. Continuing.", e)
			continue
		}

		utils.Logger.Debug().Msgf("Solving for element %d", e)
		utils.Logger.Trace().Msgf("Element %d's eligible constraints: {", e)
		for _, constraint := range c {
			utils.Logger.Trace().Msgf("%d, ", constraint.GetID())
		}
		utils.Logger.Trace().Msg("}")
		if len(c) < 2 {
			utils.Logger.Error().Msgf("Could not find a constraint to solve with %d constraints left to solve", len(g.constraints)-g.solved.Count())
		...
		utils.Logger.Debug().Msgf("Solving constraints %d %d", c[0].GetID(), c[1].GetID())
		if s := solver.SolveConstraints(c[0], c[1], g.elements[e]); state == solver.Solved {
			utils.Logger.Trace().Msgf("solve state changed to %v", s)
			utils.Logger.Debug().Msgf("solved element %v", g.elements[e])
			element, _ := c[0].Element(e)
			utils.Logger.Trace().Msgf("solved element in constraint 0: %v", element)
			element, _ = c[1].Element(e)
			utils.Logger.Trace().Msgf("solved element in constraint 1: %v", element)
			state = s
			utils.Logger.Trace().Msgf("State: %v", state)
		}
		...
		utils.Logger.Info().Msgf("Local Solve Step 3 (check for completion) %d / %d solved", g.solved.Count(), len(g.constraints))
	}

	utils.Logger.Info().Msgf("finished with state %v", state)
	g.logElements(zerolog.InfoLevel)
	return state
}
...
func (g *GraphCluster) logElement(e el.SketchElement, level zerolog.Level) {
	...
	line := e.AsLine()
	if point == nil {
		utils.Logger.WithLevel(level).Msgf("element %d: %fx + %fy + %f = 0", line.GetID(), line.GetA(), line.GetB(), line.GetC())
		return
	}
	utils.Logger.WithLevel(level).Msgf("element %d: (%f, %f)", point.GetID(), point.GetX(), point.GetY())
}
...
func (g *GraphCluster) mergeOne(other *GraphCluster, mergeConstraints bool) solver.SolveState {
	...
	utils.Logger.Debug().Msgf("Shared elements %v", sharedElements)
...
func (g *GraphCluster) solveMerge(c1 *GraphCluster, c2 *GraphCluster) solver.SolveState {
	...
	utils.Logger.Trace().Msgf("Solving for shared elements %v", sharedElements)
	...
	utils.Logger.Trace().Msgf("root cluster is c%d", finalIndex)

	for _, se := range sharedElements {
		parents := orderClustersFor(se)
		if len(parents) != 2 {
			utils.Logger.Error().Msgf("Shared element %d has %d parents. Returning Non-Convergent", se, len(parents))
			return solver.NonConvergent
		}
		...
		utils.Logger.Trace().Msgf("Solving for element %d (%v)", se, eType)
		...
		utils.Logger.Trace().Msgf("Solved for element %d:", se)
	...
	utils.Logger.Trace().Msgf("Solved %d, %d leaving final element %d", e[0], e[1], final)
	g.logElements(zerolog.TraceLevel)
	utils.Logger.Trace().Msg("")
	c1.logElements(zerolog.TraceLevel)
	utils.Logger.Trace().Msg("")
	...
	e2Type := p0Final.GetType()
	utils.Logger.Trace().Msgf("Final element type: %v", e2Type)
	if e2Type == el.Line {
		...
		p0Final = parents[0].elements[final]
		p1Final = parents[1].elements[final]
		if !p0Final.AsLine().IsEquivalent(p1Final.AsLine()) {
			utils.Logger.Error().Msg("Lines are not equivalent: ")
			utils.Logger.Error().Msgf("(%d): %v", p0Final.GetID(), p0Final)
			utils.Logger.Error().Msgf("(%d): %v", p1Final.GetID(), p1Final)
			state = solver.NonConvergent
		}
	...
	if e, ok := parents[0].elements[e[0]]; ok {
		e1 = e
		dist := p0Final.DistanceTo(e)
		constraint1 = constraint.NewConstraint(0, constraint.Distance, p0Final, e, dist, false)
		utils.Logger.Trace().Msgf("Creating constraint from %d to %d with distance %f", p0Final.GetID(), e.GetID(), dist)
	}
	if e, ok := parents[1].elements[e[0]]; ok {
		e2 = e
		dist := p1Final.DistanceTo(e)
		constraint1 = constraint.NewConstraint(0, constraint.Distance, p1Final, e, dist, false)
		utils.Logger.Trace().Msgf("Creating constraint from %d to %d with distance %f", p1Final.GetID(), e.GetID(), dist)
	}
	if e, ok := parents[0].elements[e[1]]; ok {
		e1 = e
		dist := p0Final.DistanceTo(e)
		constraint2 = constraint.NewConstraint(0, constraint.Distance, p0Final, e, dist, false)
		utils.Logger.Trace().Msgf("Creating constraint from %d to %d with distance %f", p0Final.GetID(), e.GetID(), dist)
	}
	if e, ok := parents[1].elements[e[1]]; ok {
		e2 = e
		dist := p1Final.DistanceTo(e)
		constraint2 = constraint.NewConstraint(0, constraint.Distance, p1Final, e, dist, false)
		utils.Logger.Trace().Msgf("Creating constraint from %d to %d with distance %f", p1Final.GetID(), e.GetID(), dist)
	}

	newE3, state := solver.ConstraintResult(constraint1, constraint2, p0Final)
	newP3 := newE3.AsPoint()
	...
	utils.Logger.Trace().Msgf("Desired merge point for c1 and c2: %f, %f", newP3.GetX(), newP3.GetY())
	...
	utils.Logger.Trace().Msgf("Pivoting c0 on %d from %v to %v", e1.GetID(), p0Final, newP3)
	moveCluster(parents[0], e1, p0Final.AsPoint(), newP3)
	utils.Logger.Trace().Msgf("parent 0 final moved to %v", p0Final)
	utils.Logger.Trace().Msgf("Pivoting c1 on %d from %v to %v", e2.GetID(), p1Final, newP3)
	moveCluster(parents[1], e2, p1Final.AsPoint(), newP3)
	utils.Logger.Trace().Msgf("parent 1 final moved to %v", p1Final)
...
func (g *GraphCluster) Solve() solver.SolveState {
	utils.Logger.Info().Msgf("Solving cluster %d", g.id)
	state := g.localSolve()
	return state
}
...
func (c *GraphCluster) IsSolved() bool {
	...
	for _, c := range c.constraints {
		...
		utils.Logger.Trace().Msgf("Failed to meet %v", c)
		solved = false
	}
